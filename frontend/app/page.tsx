...
const API_BASE_URL = "https://api.gitlift.in";
// ------------------------------------------------------------------
...
export default function Home() {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      const targetUrl = url || `https://${projectSlug}.gitlift.in`;

      // ⚡ CHECK IF BUILD WAS SKIPPED
      if (data.status === "skipped") {
        setDeployUrl(targetUrl);
        setLogs((prev) => [
          ...prev,
          `⚡ No changes detected for ${projectSlug}. Skipping build!`,
          `✨ Fast-forwarded to existing deployment.`
        ]);
        setStatus("deployed"); // Instantly show the "Visit Website" button!
        return; // Exit early—no need to wait for Socket.io build logs
      }
